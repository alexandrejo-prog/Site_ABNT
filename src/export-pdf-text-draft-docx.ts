...
function visualKindLabel(kind?: PdfLayoutSensitiveRegionDiagnostic["kind"]): string {
  ...
  if (kind === "grafico") return "Gr├ífico";
  if (kind === "imagem") return "Imagem";
  if (kind === "mapa") return "Mapa";
  if (kind === "ilustracao") return "Ilustra├º├úo";
  if (kind === "multicolumn") return "Conte├║do em colunas";
  return "Elemento visual n├úo identificado";
}

function originalPagesLabel(pageStart: number, pageEnd: number): string {
  return pageStart === pageEnd ? `p├ígina original ${pageStart}` : `p├íginas originais ${pageStart}-${pageEnd}`;
}
...
function markerForBlock(block: PdfReconstructedBlockDiagnostic, regions: Map<string, PdfLayoutSensitiveRegionDiagnostic>, logicalRange?: { pageStart: number; pageEnd: number }): string {
  if (!block.layoutRegionId) return `[Conte├║do com estrutura visual n├úo resolvida, p├ígina original ${block.pageStart}. Consulte o PDF.]`;
  const region = regions.get(block.layoutRegionId);
  if (!region) return `[Conte├║do com estrutura visual n├úo resolvida, p├ígina original ${block.pageStart}. Consulte o PDF.]`;
  const pageStart = logicalRange?.pageStart ?? region.pageStart;
  const pageEnd = logicalRange?.pageEnd ?? region.pageEnd;
  return `[Elemento visual n├úo inserido neste rascunho textual - ${visualKindLabel(region.kind)}, ${originalPagesLabel(pageStart, pageEnd)}. Consulte o PDF original.]`;
}
...
function coverParagraphs(input: PdfTextDraftExportInput, logo?: PdfTextDraftLogoAsset): Paragraph[] {
  ...
  const title = cover?.title ?? "[T├ìTULO AUSENTE]";
...
function titlePageParagraphs(input: PdfTextDraftExportInput): Paragraph[] {
  ...
  const title = titlePage?.title ?? cover?.title ?? "[T├ìTULO AUSENTE]";
...
function makeTocEntries(blocks: PdfReconstructedBlockDiagnostic[]): TocEntry[] {
  return blocks
    ...
    .filter((block) => /^(?:\d+(?:\.\d+)*\s+\S|REFER[├èE]NCIAS|AP[├èE]NDICE|ANEXO)/i.test(cleanText(block.text)))
    .map((block, index) => ({
...
function tocParagraphs(entries: TocEntry[]): Paragraph[] {
  return [
    centered("SUM├üRIO", { bold: true }),
    left("Atualize os campos do sum├írio no Word com Ctrl+A e F9.", { size: 20, italics: true }),
...
const VISUAL_CAPTION_RE = /^(Quadro|Tabela|Figura|Gr[├ía]fico|Imagem|Mapa|Ilustra[├ºc][├úa]o)\s+\d+\s*[-ÔÇôÔÇö.:]/iu;
  ...
  for (const block of input.reconstruction.blocks) {
    ...
    if (block.type === "caption") {
      ...
      if (region && !emittedMarkerKeys.has(dedupKey)) {
        ...
        if (hasUnresolved) {
          // Imagens e marcadores da regi├úo com bloco unresolved s├úo emitidos na posi├º├úo do unresolved.
...
export function validatePdfTextDraftExport(input: PdfTextDraftExportInput): PdfTextDraftValidation {
  ...
  if (input.documentMode !== "pdf-text-draft") blockers.push("O modo de exporta├º├úo precisa ser pdf-text-draft.");
  if (!input.reconstruction.bodyStart.found) blockers.push("O in├¡cio do corpo textual n├úo foi identificado.");
  if (input.reconstruction.blocks.length === 0) blockers.push("Nenhum bloco reconstru├¡do foi encontrado.");
  if (!input.reconstruction.blocks.some((block) => block.type === "paragraph")) blockers.push("Nenhum par├ígrafo reconstru├¡do foi encontrado.");
  if (input.reconstruction.blocks.some((block) => block.type === "paragraph" && block.pageEnd - block.pageStart > 1)) blockers.push("H├í par├ígrafo atravessando mais de duas p├íginas.");
  ...
  if (stats.unresolvedCount > 0) warnings.push("H├í blocos visuais n├úo resolvidos que ser├úo representados por marcadores.");
  if (stats.uncertainHyphenationCount > 0) warnings.push("H├í hifeniza├º├Áes incertas para revis├úo.");
  if (stats.lowConfidenceBlockCount > 0) warnings.push("H├í blocos de baixa confian├ºa.");
  if (stats.layoutRegionCount > 0) {
    if (input.visualAssets && Object.keys(input.visualAssets).length > 0) {
      warnings.push("Alguns elementos visuais podem permanecer como marcadores quando o recorte autom├ítico n├úo estiver dispon├¡vel.");
    } else {
      warnings.push("Elementos visuais ser├úo representados por marcadores.");
    }
  }
  if (!input.pretextual?.abstract) warnings.push("Abstract ausente: nenhum texto ser├í inventado.");
  if (!input.reconstruction.layoutRegions.some((region) => region.kind === "figura" || region.kind === "grafico")) warnings.push("Nenhuma figura ou gr├ífico foi detectado textualmente.");
  if (stats.multiPageParagraphCount > 0) warnings.push("H├í par├ígrafos atravessando at├® duas p├íginas.");
