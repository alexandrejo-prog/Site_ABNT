...
function patchGraduateDocumentXml(xml: string): string {
  return xml
    .replaceAll(CATALOG_PLACEHOLDER_BEFORE, CATALOG_PLACEHOLDER_AFTER)
    .replaceAll(APPROVAL_PLACEHOLDER_BEFORE, APPROVAL_PLACEHOLDER_AFTER)
    .replaceAll('<w:ind w:firstLine="0" w:left="0"/>', '<w:ind w:left="0" w:hanging="720"/>');
}
