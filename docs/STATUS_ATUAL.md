...
21. [x] **Fidelidade do preview vs DOCX renderizado (diff visual)** — `compare-preview-docx.ts`: gera a monografia (rascunho editável), renderiza via Word COM e compara o HTML do preview com o PDF — similaridade de tokens 0.833 ≥ 0.65, Δpáginas 1 ≤ 3 (gate `previewDiffGate`, 9/9 no total); evidência visual lado a lado (screenshots docx × preview × diff via pdfjs+@napi-rs/canvas+pixelmatch e Playwright) em `artifacts/ufla-compliance/preview-diff/*.png` (diffRatio página 1: 0.006); artefato `preview-docx-diff.json`; sem Word o gate passa de forma graciosa.
