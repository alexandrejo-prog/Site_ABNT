...
23b. [x] **Resíduo conhecido de numeração (documentado)** — o preview numera o corpo pela contagem física (monografia: corpo em 8) enquanto o DOCX usa `calculateTextualStartPage` (folha de rosto=1, capa/ficha no verso não contam → corpo em 6); o TOC renderizado pelo Word mistura os dois (pré-textuais físicos 3,5,6,7; corpo numerado 6,7,8). Decisão registrada: manter os dois modelos (o preview é uma simulação de paginação; o digest do DOCX garante que o `w:start` não regrida).
