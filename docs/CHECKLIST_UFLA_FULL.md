...
| 15 | Numeração de páginas a partir da folha de rosto, visível a partir da introdução | §3.2.7 | ✅ | `scripts/ufla-compliance/validate-pagination.ts` | UFLA-AMBIGUOUS-1 resolvida (DECISION-010): contagem contínua (folha de rosto = 1), visível a partir da Introdução com o valor contado (pgNumType start=13 ↔ PDF físico folha 18=13), nunca reinício em 1; OOXML + PDF físico; gate PASSED |
...
| 67 | Tipo de trabalho específico: Artigo científico não deve ter capa UFLA, folha de rosto, ficha catalográfica, sumário, paginação | §3.1 | ✅ | `DOCUMENT_TYPE_MATRIX` + `audit-pretextual.ts` (por tipo) + `audit-formats-cross.ts` | Regras pertinentes de artigo/CPG definidas na matriz (resumo/abstract/introdução/desenvolvimento/referências/layout/tipografia/espaçamento); gate por tipo 4/4 PASSED |
| 68 | Tipo de trabalho específico: Resumo Expandido CPG não deve ter capa, folha de rosto, sumário, paginação, apêndices/anexos | §3.1 | ✅ | `DOCUMENT_TYPE_MATRIX` + `audit-pretextual.ts` (por tipo) + `audit-formats-cross.ts` | idem — matriz de tipos e gate por tipo (gates-per-type.json) |
