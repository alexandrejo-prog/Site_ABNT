...
describe("Inspecao profunda DOCX vs Manual UFLA 6ed", () => {
  ...
  it("1. Margens e espacamento", () => {
    ...
    expect(hasHeadingAtLevel(xml, stylesXml, 1, "1 INTRODUCAO")).toBe(true);
    const firstNumberedPrimary = headingParagraphsAtLevel(xml, stylesXml, 1).find((p) =>
      p.includes('w:val="ufla_titulo_primario"'),
    );
    expect(firstNumberedPrimary).toBeTruthy();
    expect(firstNumberedPrimary).toContain('w:before="240"');
    report.push(
  ...
  it("4. Pos-textuais como titulos sem indicativo centralizados", () => {
    ...
    // Verify they are level-1 titles (sem indicativo) with center
    for (const title of ["REFERENCIAS", "ANEXOS"]) {
      ...
      if (idx >= 0) {
        ...
        expect(paraXml).toMatch(/w:val="ufla_(titulo_sem_indicativo|anexo_titulo|apendice_titulo)"/);
  ...
  it("9. Titulo primario numerado esquerda, sem-indicativo centralizado, heading2/3 em negrito", () => {
    const h1Paras = headingParagraphsAtLevel(xml, stylesXml, 1);
    expect(h1Paras.length).toBeGreaterThan(0);
    for (const h1 of h1Paras) {
      const styleId = h1.match(/<w:pStyle\b[^>]*w:val="([^"]+)"/)?.[1] ?? "";
      if (styleId === "ufla_titulo_primario") {
        expect(h1).toContain('w:val="left"');
      } else {
        // sem_indicativo / anexo / apendice: centralizados
        expect(h1).toContain('w:val="center"');
      }
    }
    const h2Paras = headingParagraphsAtLevel(xml, stylesXml, 2);
    expect(h2Paras.length).toBeGreaterThan(0);
    const h3Paras = headingParagraphsAtLevel(xml, stylesXml, 3);
    if (h3Paras.length > 0) {
    ...
    report.push(
      { item: "9. Titulo primario numerado esquerda / sem-indicativo centro", status: "OK", obs: "" },
  ...
  afterAll(() => {
    ...
    // Relatório vai para o diretório temporário de evidências — não sobrescreve
    // o relatório oficial do repositório.
    const outDir = testEvidenceDir();
