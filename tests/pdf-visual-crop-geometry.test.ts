...
describe("camada de geometria de recorte visual pdf", () => {
  ...
  describe("delimitacao por limites estruturais (legenda/fonte/mancha)", () => {
    it("quadro alto nao e reduzido a uma faixa estreita quando o limite estrutural continua", () => {
      const rows = Array.from({ length: 14 }, (_, i) =>
        line(1, { left: 100, right: 900, top: 160 + i * 60, bottom: 190 + i * 60, height: 30, text: "linha" }),
      );
      const pages = [page(1, PAGE_W, PAGE_H, [
      ...
      const regions = [region({ id: "r1", pageStart: 1, pageEnd: 1, startLineIndex: 0, endLineIndex: 15, kind: "quadro", caption: "Quadro 1 – Título.", source: "Fonte: A." })];
      const { crops } = computePdfVisualCropGeometry(pages, regions);
    ...
    it("quadro de pagina unica usa legenda e fonte como limites verticais", () => {
      const pages = [page(1, PAGE_W, PAGE_H, [
    ...
    it("quadro multipagina gera recorte de primeira, intermediaria e ultima pagina", () => {
      const pages = [
        page(1, PAGE_W, PAGE_H, [
          line(1, { left: 100, right: 900, top: 100, bottom: 130, height: 30, text: "Quadro 1 – M." }),
          line(1, { left: 100, right: 900, top: 200, bottom: 230, height: 30, text: "a" }),
          line(1, { left: 100, right: 900, top: 260, bottom: 290, height: 30, text: "b" }),
          line(1, { left: 100, right: 900, top: 320, bottom: 350, height: 30, text: "c" }),
        ]),
        page(2, PAGE_W, PAGE_H, [
          line(2, { left: 100, right: 900, top: 200, bottom: 230, height: 30, text: "d" }),
          line(2, { left: 100, right: 900, top: 260, bottom: 290, height: 30, text: "e" }),
          line(2, { left: 100, right: 900, top: 320, bottom: 350, height: 30, text: "f" }),
        ]),
        page(3, PAGE_W, PAGE_H, [
          line(3, { left: 100, right: 900, top: 200, bottom: 230, height: 30, text: "g" }),
          line(3, { left: 100, right: 900, top: 260, bottom: 290, height: 30, text: "h" }),
          line(3, { left: 100, right: 900, top: 320, bottom: 350, height: 30, text: "i" }),
          line(3, { left: 100, right: 900, top: 1040, bottom: 1070, height: 30, text: "Fonte: A." }),
        ]),
      ];
      const regions = [region({ id: "r1", pageStart: 1, pageEnd: 3, startLineIndex: 0, endLineIndex: 3, kind: "quadro", caption: "Quadro 1 – M.", source: "Fonte: A." })];
      const { crops } = computePdfVisualCropGeometry(pages, regions);
      expect(crops.map((c) => c.pageNumber)).toEqual([1, 2, 3]);
      const first = crops.find((c) => c.pageNumber === 1)!;
      const last = crops.find((c) => c.pageNumber === 3)!;
    ...
    it("legenda quebrada em duas linhas e excluida do recorte", () => {
      const pages = [page(1, PAGE_W, PAGE_H, [
      ...
      expect(crops).toHaveLength(1);
      const c = crops[0];
      expect(c.sourceRect.y).toBe(169); // base da segunda linha da legenda (165) + gap
      expect(c.sourceRect.y).toBeGreaterThan(165); // ambas as linhas da legenda fora
      expect(c.sourceRect.y).toBeLessThan(200); // conteudo preservado
    });

    it("topo da tabela nao e cortado na primeira pagina", () => {
      const pages = [page(1, PAGE_W, PAGE_H, [
        line(1, { left: 100, right: 900, top: 100, bottom: 130, height: 30, text: "Quadro 1 – M." }),
        line(1, { left: 100, right: 900, top: 160, bottom: 190, height: 30, text: "primeira linha" }),
        line(1, { left: 100, right: 900, top: 220, bottom: 250, height: 30, text: "segunda linha" }),
      ])];
      const regions = [region({ id: "r1", pageStart: 1, pageEnd: 1, startLineIndex: 0, endLineIndex: 2, kind: "tabela", caption: "Quadro 1 – M." })];
      const { crops } = computePdfVisualCropGeometry(pages, regions);
      expect(crops).toHaveLength(1);
      // 130 + 4 = 134 <= topo da primeira linha (160): nao corta
    ...
    it("altura de 32px continua rejeitada como implausivel (envlope sem legenda/fonte)", () => {
      const pages = [page(1, PAGE_W, PAGE_H, [
