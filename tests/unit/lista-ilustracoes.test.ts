...
describe("Lista de Tabelas", () => {
  ...
  it("TODO PAGEREF deve ter um bookmark correspondente (sem 'Erro! Indicador não definido')", async () => {
    const documentXml = await documentXmlFor();

    // Extrai cada instrução PAGEREF <PAGEREF NOME \h> e cada nome de bookmarkStart.
    const pagerefTargets = Array.from(documentXml.matchAll(/PAGEREF\s+([^\s\\"]+)/g), (m) => m[1]);
    const bookmarkNames = new Set(
      Array.from(documentXml.matchAll(/<w:bookmarkStart[^>]*w:name="([^"]+)"/g), (m) => m[1]),
    );

    expect(pagerefTargets.length).toBeGreaterThan(0);
    for (const target of pagerefTargets) {
      expect(bookmarkNames.has(target), `PAGEREF '${target}' sem BookmarkStart correspondente`).toBe(true);
    }

    // Nenhum bookmarkStart duplicado (ambíguo) com o mesmo nome.
    const names = Array.from(documentXml.matchAll(/<w:bookmarkStart[^>]*w:name="([^"]+)"/g), (m) => m[1]);
    const seen = new Map<string, number>();
    for (const n of names) seen.set(n, (seen.get(n) ?? 0) + 1);
    for (const [n, count] of seen) {
      expect(count, `BookmarkStart '${n}' aparece ${count} vezes`).toBe(1);
    }
  });

  it("legenda com 'Fonte:' gera PAGEREF apontando para bookmark sem o trecho 'Fonte'", async () => {
    // O corpo envolve a legenda (sem 'Fonte:') com o bookmark; a lista referencia o MESMO id.
    const documentXml = await documentXmlFor();
    const bookmarkNames = new Set(
      Array.from(documentXml.matchAll(/<w:bookmarkStart[^>]*w:name="([^"]+)"/g), (m) => m[1]),
    );
    ...
    for (const target of pagerefTargets) {
      expect(bookmarkNames.has(target), `bookmark '${target}' ausente`).toBe(true);
    }
  });
});
